# installer/sanjinsight_linux.spec
# -*- mode: python ; coding: utf-8 -*-
#
# PyInstaller spec for the Linux AppImage.
# Run from the project root:
#   pyinstaller installer/sanjinsight_linux.spec --noconfirm --clean
#
# Output: dist/SanjINSIGHT
...
hidden_imports = [
    ...
    'hardware.arduino.nano_driver',
    'hardware.arduino.simulated',
    'hardware.autofocus.sweep',
    'hardware.autofocus.hill_climb',
    'hardware.autofocus.simulated',

    # ── Debug logging + error narration helpers ──────────────────────
    'hardware.hw_debug_log',
    'hardware.error_narration',
    ...
    *_pypylon_modules,
    ...
    'serial.tools.list_ports_linux',   # Linux-specific port enumeration
...
datas = [
    ...
    # Used by hardware/cameras/ni_imaqdx.py on Windows.  Bundled on Linux too
    ...
    *( [(os.path.join(_root, 'firmware', 'ez500_fpga.lvbitx'), 'firmware')]
       if os.path.isfile(os.path.join(_root, 'firmware', 'ez500_fpga.lvbitx'))
       else [] ),
...
a = Analysis(
    ...
    runtime_hooks=[],
    excludes=[
        # macOS/Windows-only — never present on Linux
        'nifpga',
        'win32api', 'win32con', 'win32gui', 'pywintypes',
        'winreg', 'winsound',
        'AppKit', 'CoreFoundation', 'objc',
...
# ── COLLECT (assemble into one directory) ─────────────────────────────────────
coll = COLLECT(
    exe,
    a.binaries,
    a.zipfiles,
