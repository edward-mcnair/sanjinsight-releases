...
class MainWindow(QMainWindow):
    ...
    def _connect_signals(self):
        ...
        # All hardware-thread signals use QueuedConnection to ensure
        # slots execute on the GUI thread, never blocking the emitter.
        signals.new_live_frame.connect(self._on_frame, Qt.QueuedConnection)
        signals.tec_status.connect(self._on_tec, Qt.QueuedConnection)
        signals.fpga_status.connect(self._on_fpga, Qt.QueuedConnection)
        signals.bias_status.connect(self._on_bias, Qt.QueuedConnection)
        signals.stage_status.connect(self._on_stage, Qt.QueuedConnection)
        signals.af_progress.connect(self._on_af_progress, Qt.QueuedConnection)
        signals.af_complete.connect(self._on_af_complete, Qt.QueuedConnection)
        signals.cal_progress.connect(self._on_cal_progress, Qt.QueuedConnection)
        signals.cal_complete.connect(self._on_cal_complete, Qt.QueuedConnection)
        signals.scan_progress.connect(self._on_scan_progress, Qt.QueuedConnection)
        signals.scan_complete.connect(self._on_scan_complete, Qt.QueuedConnection)
        signals.profile_applied.connect(self._on_profile_applied)
        signals.acq_progress.connect(self._on_acq_progress, Qt.QueuedConnection)
        signals.acq_complete.connect(self._on_acq_complete, Qt.QueuedConnection)
        signals.acq_saved.connect(self._on_acq_saved)
        # AutoScan live + result feeds
        signals.new_live_frame.connect(self._autoscan_tab.on_live_frame, Qt.QueuedConnection)
        signals.acq_progress.connect(self._autoscan_tab.on_acq_progress, Qt.QueuedConnection)
