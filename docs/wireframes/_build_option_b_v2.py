#!/usr/bin/env python3
"""Option B v2 — Phase-Aware Sidebar with updated labels and SYSTEM group."""
import sys, os
...
OUT = os.path.join(os.path.dirname(__file__), "option-b-v2-phase-sidebar.pdf")

c = Canvas(OUT, pagesize=(W, H))
c.setTitle("Option B v2 — Phase-Aware Sidebar")
...
for phase in phases:
    ...
    text_at(c, 32, indicator_cy + 1, phase["title"], size=9.5,
    ...
    for item_label, selected in phase["items"]:
        item_h = 28
        if selected:
            # Selected item — accent background + left bar
    ...
    sy += 6  # gap between phases

# ── Separator line before SYSTEM ──────────────────────────────────────
sy += 2
hline(c, 12, sidebar_w - 12, sy, color=BORDER, width=1)
sy += 8

# ── SYSTEM group (no phase number, no progress indicator) ─────────────
# Header
...
gear_x = 18
gear_cy = sy + 13
c.drawCentredString(gear_x, Y(gear_cy) - 3.5, "\u2699")

text_at(c, 32, gear_cy + 1, "SYSTEM", size=9.5,
...
for item_label, selected in system_items:
    item_h = 28
    text_at(c, 28, sy + item_h / 2 + 1, item_label,
...
# Camera info badge (bottom left of preview)
...
text_at(c, main_x + main_w / 2, content_bottom - 10,
        "OPTION B v2  —  Phase-Aware Sidebar", size=8, color=TEXT_SUB,
        font="Helvetica-Bold", anchor="center")
