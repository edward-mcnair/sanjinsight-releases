...
class NewportNPC3Driver(StageDriver):
    ...
    def _send(self, cmd: str) -> str:
        ...
        CR+LF.  Responses are also terminated with CR or CR+LF.
        """
        ...
        # Read response — some commands echo, some return data, some
        # return nothing.  Read with a short timeout.
    ...
    def _set_channels(self,
        ...
        if x is not None and y is not None and z is not None:
            self._send(f"setall,{x:.3f},{y:.3f},{z:.3f}")
        else:
            if x is not None:
                self._send(f"set,{_CH_X},{x:.3f}")
            if y is not None:
                self._send(f"set,{_CH_Y},{y:.3f}")
            if z is not None:
                self._send(f"set,{_CH_Z},{z:.3f}")
