...
class ArduinoNanoDriver(ArduinoDriver):
    ...
    def _auto_detect_port() -> Optional[str]:
        ...
        _NANO_VIDS = {0x1A86, 0x0403, 0x2341}  # CH340, FTDI, Arduino.cc
        _NANO_PIDS = {0x7523, 0x6001, 0x0043}  # CH340, FTDI, Uno/Nano

        for port_info in comports():
            vid = port_info.vid
            pid = port_info.pid
            if vid is not None and pid is not None:
                if vid in _NANO_VIDS and pid in _NANO_PIDS:
                    log.info("Auto-detected Arduino Nano on %s "
                             "(VID=%04X PID=%04X)",
            ...
            if "ch340" in desc or "arduino nano" in desc:
                log.info("Auto-detected Arduino Nano on %s (desc=%s)",
